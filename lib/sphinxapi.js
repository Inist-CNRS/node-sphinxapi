...
function SphinxClient() { 
	...
	this.host			= 'localhost'					// searchd host (default is "localhost")
	this.port			= 9312							// searchd port (default is 9312)
	this.path			= null							// searchd unix-domain socket path
	this.socket			= null
	this.offset			= 0								// how much records to seek from result-set start (default is 0)
	this.limit			= 20							// how much records to return from result-set starting at offset (default is 20)
	this.mode			= SphinxClient.SPH_MATCH_ALL			// query matching mode (default is SPH_MATCH_ALL)
	this.weights		= []							// per-field weights (default is 1 for all fields)
	this.sort			= SphinxClient.SPH_SORT_RELEVANCE		// match sorting mode (default is SPH_SORT_RELEVANCE)
	this.sortby			= ''							// attribute to sort by (defualt is "")
	this.min_id			= 0								// min ID to match (default is 0)
	this.max_id			= 0								// max ID to match (default is UINT_MAX)
	this.filters		= []							// search filters
	this.groupby		= ''							// group-by attribute name
	this.groupfunc		= SphinxClient.SPH_GROUPBY_DAY			// group-by function (to pre-process group-by attribute value with)
	this.groupsort		= '@group desc'					// group-by sorting clause (to sort groups in result set with)
	this.groupdistinct	= ''							// group-by count-distinct attribute
	this.maxmatches		= 1000							// max matches to retrieve
	this.cutoff			= 0								// cutoff to stop searching at
	this.retrycount		= 0								// distributed retry count
	this.retrydelay		= 0								// distributed retry delay
	this.anchor			= {}							// geographical anchor point
	this.indexweights	= {}							// per-index weights
	this.ranker			= SphinxClient.SPH_RANK_PROXIMITY_BM25  // ranking mode
	this.rankexpr		= ''							// ranking expression for SPH_RANK_EXPR
	this.maxquerytime	= 0								// max query time, milliseconds (default is 0, do not limit)
	this.timeout		= 1.0							// connection timeout
	this.fieldweights	= {}							// per-field-name weights
	this.overrides		= {}							// per-query attribute values overrides
	this.select			= '*'							// select-list (attributes or expressions, with optional aliases)
...
SphinxClient.prototype._SetRequest = function(client_ver, request, fn) {
	...
	client.once('data', function(chunk) {
			...
			if (!Array.isArray(response)) {
				fn(new Error('connection to '+_host+':'+_port+' failed'))
			...
			client.on('data', function(chunk) {
					...
					if (content.length >= length) {
						...
						if (state == SphinxClient.SEARCHD_WARNING) {
							fn(new Error('Warning: '+content.toString()))
						}
						if (state == SphinxClient.SEARCHD_ERROR) {
							fn(new Error('searchd error: '+content.toString()))
						}

						if (state == SphinxClient.SEARCHD_RETRY) {
							fn(new Error('temporary searchd error: '+content.toString()))
						}

						if (state != SphinxClient.SEARCHD_OK) {
...
SphinxClient.prototype.Status = function(fn) {
	...
	request = bp.pack( '>2HLL', [this.SEARCHD_COMMAND_STATUS, SphinxClient.VER_COMMAND_STATUS, 4, 1])
