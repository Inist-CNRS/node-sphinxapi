...
SphinxClient.prototype.RunQueries = function(fn) {
	...
	this._SetRequest(SphinxClient.VER_COMMAND_SEARCH, request, function(err, response) {
		...
		for (i = 0; i < nreqs; i++) {
			...
			result['warning'] = ''
			/*
			status = bp.unpack('>L', response[p:p+4])[0]
			p += 4
...
//    request1 = bp.pack( '>2H', [SphinxClient.SEARCHD_COMMAND_STATUS, SphinxClient.VER_COMMAND_STATUS])
//    request2 = bp.pack( '>2L', [4, 1])
//    var req = []
//    req.push(request1);
//    req.push(request2);
