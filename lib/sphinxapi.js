...
SphinxClient.prototype.AddQuery = function(query, index, comment) {
	...
	*/
	...
	// anchor point
	if (this._anchor.length == 0) {
		req.push(bp.pack('>L', 0))
	}
	else {
		req.push(pb.pack('>L', 1))
		req.push(pb.pack('>L', this._anchor.attrlat.length) + this._anchor.attrlat)
		req.push(pb.pack('>L', this._anchor.attrlong.length) + this._anchor.attrlong)
		req.push(pb.pack('>f', this._anchor.lat) + pb.pack ('>f', this._anchor.long))
	}

	// per-index weights
	req.push(bp.pack('>L', this._indexweights.length))

	forEach(this._indexweights, function(index, weight) {
			req.push(bp.pack('>L', index.length) + index + bp.pack('>L', weight))
	});
	...
	// per-field weights
	req.push(bp.pack('>L', this._fieldweights.length))
	forEach(this._fieldweights, function(field, weight) {
			req.push(pb.pack('>L', field.length) + field + bp.pack('>L', weight))
	});

	// comment
	req.push(pb.pack('>L', comment.length) + comment)

	// attribute overrides
	req.push(pb.pack('>L', this._overrides.length) )
	
	forEach(this._overrides, function(index, v) {
			req.push(bp.pack('>L', v['name'].length) + v['name'])
			req.push(bp.pack('>LL', v['type'], v['values'].length) )
			v['values'].forEach(function(value, id) {
		...
		// select-list
		req.push(bp.pack('>L', this._select.length))
		req.append(this._select)
